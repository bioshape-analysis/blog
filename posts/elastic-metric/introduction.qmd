...
$${#eq-length}
and the \emph{geodesic} \emph{distance} between $c_0$ and $c_1$ is
$$
...
While the procedure to construct the mapping can be numerically unstable \cite{bauer2014constructing,needham2020simplifying}, it is simple for the SRV, with the geodesic distance being the $\mathcal{L}^2$ distance obtained upon representing the curve by its speed, renormalized by the square root of its norm as
$q(c) = \dot{c}/\sqrt{\lvert \dot{c} \rvert}$ [@bauer2022elastic].

### Fréchet mean
With the space of curves equipped with this distance, the so-called Fréchet mean of $n$ curves $(c_1,\ldots,c_n)$ \cite{miolane2020introduction} is defined as
$$
