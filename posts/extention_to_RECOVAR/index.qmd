...
We've present the results on our SARS-CoV2 spike dataset output by original RECOVAR pipeline in the previous blog. Here for comparison with the modified algorithms in the later sections, we showed the complete movie of one RBD transiting from open state with ACE2 to close state:
