...
Both of the methods that we are going to cover are iterative methods relying on an initial sample of points. For generating a point cloud with size $n$, they begin by randomly sampling $n$ points and refining it over iterations. To make the illustration interesting we used a uniform point cloud for initial samples. We use $n=200$ in our examples.
