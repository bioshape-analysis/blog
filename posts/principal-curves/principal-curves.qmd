...
y\rvert.$$
In <span style='color: red;'>our preprint (Cor. 4.10)</span> we
essentially showed that given $f \in C(X; \Omega)$ and $\xi \in C(X;
\mathbb R^d)$, modulo some technical details one has
$$
\begin{equation}
...
d\mu(\omega). \tag{2} \label{eq:first-var}
\end{equation}
$$
An analogous formula for the discretized case is found in <span
style='color: red;'>(Prop. 5.5)</span>. In this case, one may show
that the difference quotient $\eqref{eq:first-var}$ is minimized when
$\xi(y_j)$ is defined to be
$$
...
soon-to-be-released preprint <span style='color: red;'>insert
reference to Andrew project</span>). First, we compute the Voronoi
...
element of $\mathrm{Spl}_{k+1}(T; \mathbb R^d)$ interpolating the
provided points $\{y_1, \ldots, y_N\}$


<!-- of the points $\{f(t_1), -->
<!-- \ldots, f(t_N)\}$. The particular family of interpolations we are -->
<!-- interested in is those that can be expressed as piecewise polynomials of degree at most $k$. -->

<!--  interpolations of a prescribed collection of points -->
<!-- $\{f(t_1), \ldots, f(t_N)\}$ via piecewise polynomials of degree at -->
<!-- most $k$. Note this guarantees the interpolation will have continuous -->
<!-- derivatives of order up to $k-1$. -->

<!-- Note that because we've required the spline to _interpolate_ the -->
<!-- points $\{f(t_1), \ldots, f(t_N)\}$ -->

<!-- The idea is that we will construct the basis splines by first -->
<!-- beginning with a family of indicator functions on each interval $[t_j, -->
<!-- t_{j+1}]$. In essence, -->
